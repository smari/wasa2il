...
urlpatterns = [
    ...
    url(r'^polity/(?P<polity>\d+)/election/(?P<pk>\d+)/$',
...
#   (r'^delegation/(?P<pk>\d+)/$', login_required(DetailView.as_view(model=Delegate, context_object_name="delegation"))),
    ...
    url(r'^api/issue/comment/send/$', issue_comment_send),
    url(r'^api/issue/poll/$', issue_poll),
    url(r'^api/issue/vote/$', issue_vote),

    url(r'^api/topic/poll/$', topic_poll),

    url(r'^api/election/poll/$', election_poll),
    url(r'^api/election/vote/$', election_vote),
    url(r'^api/election/candidacy/$', election_candidacy),
